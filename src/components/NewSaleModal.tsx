...
import {
  X, Trash2, User, UserCheck, Coins, AlertTriangle, Receipt, ClipboardCheck, Search
} from 'lucide-react';
import { Product, Client, PaymentMethod, SaleStatus, SaleItem } from '../types';
...
export default function NewSaleModal({ products, clients, onClose, onRegisterSale }: NewSaleModalProps) {
  const [clientChoice, setClientChoice] = useState<string>(WALK_IN); // client id or WALK_IN
  ...
  // Lookup map kills the repeated O(n) products.find() inside cart loops.
  const productById = useMemo(() => new Map(products.map((p) => [p.id, p])), [products]);

  // Products matching the search box (by name or SKU).
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const assembledItems: SaleItem[] = cart.map((item) => {
      ...
      <div className="bg-white rounded-2xl border border-slate-200 max-w-4xl w-full shadow-2xl flex flex-col md:flex-row overflow-hidden max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 md:p-8 flex-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mb-6">
            <h4 className="text-xs font-bold text-slate-800 uppercase mb-3 text-left">Adicionar Itens do Catálogo</h4>
          ...
          <div className="flex gap-3 justify-end pt-4 border-t border-slate-100">
            ...
            <button
              type="submit"
              className="bg-gradient-to-br from-brand to-brand-mid hover:from-brand-dark hover:to-brand text-white font-bold text-xs px-6 py-2.5 rounded-xl shadow-md shadow-brand/20 active:scale-95 transition-all"
            >
              Confirmar Venda
            </button>
          </div>
